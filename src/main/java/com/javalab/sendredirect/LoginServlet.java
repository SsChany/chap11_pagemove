...
//[중요]한글 메시지 전달시 URL인코딩용 API(클래스)
import java.net.URLEncoder;
import java.net.URLDecoder;

/**
 로그인 페이지를 띄워주는 기능/로그인을 진행하는 두가지 기능
 */
@WebServlet("/login")
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String errorMsg = "";
		// doPost 메소드에서 쿼리스트링에 달아보낸 errorMsg 추출
		if (request.getParameter("errorMsg") != null) {
			/*
			[디코딩]
			오류 메시지 보내는 쪽에서 utf-8 방식으로 2byte씩 인코딩 했으므로
			받는 쪽에서도 utf-8방식으로 2byte씩 디코딩해야 완벽한 메시지복원됨.
			*/
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//아이디와 비밀번호가 같으면 인증 OK, 아니면 인중 fail 가정
		...
		if (isAuthenticated) {
			...
			/*
			[한글 메시지의 경우 인코딩]
				- 한글 메시지를 Url에 쿼리스트링 형태로 달고 가기위해 인코딩
				- Url에 달아서 보낼 메시지 인코딩(UTF-8)해서 2byte씩 처리
				- 웹페이지의 본문(메시지 바디)에서는 UTF-8을 허용하지만,
				  웹브라우저의 주소창에서는 오로지 ASCII 코드만 허용해서 문제가 됨.
